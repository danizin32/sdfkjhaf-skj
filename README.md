...
local function selectTab(name)
    ...
    tabs[name].BackgroundColor3 = Color3.fromRGB(100, 100, 100)

    if name == "Auto Farm" then createAutoFarmUI() end
    if name == "Auto Stats" then createAutoStatsUI() end
    if name == "Misc" then createMiscUI() end
end
...
function createAutoFarmUI()
    ...
    -- Estado inicial toggle AutoClick
    ...
    toggleAutoFarm.MouseButton1Click:Connect(function()
        ...
        if state.AutoFarm then
            ...
            -- Equipa a arma ao ligar AutoFarm
            equipWeapon(state.WeaponName)

            -- Liga o AutoClick junto
            ...
            -- Desliga AutoClick também quando AutoFarm desligar (opcional)
...
function createAutoStatsUI()
    ...
    local statInput = Instance.new("TextBox", tabContent)
    ...
    local label2 = Instance.new("TextLabel", tabContent)
    ...
    local amountInput = Instance.new("TextBox", tabContent)
...
spawn(function()
    while true do
        ...
        if state.AutoFarm then
            ...
            if enemy then
                ...
                if enemyHRP then
                    ...
                    hrp.CFrame = enemyHRP.CFrame * CFrame.new(0, 0, 5)
                    equipWeapon(state.WeaponName)
                    -- Ataque básico: exemplo disparar ataque a cada 1s
                    if state.AutoClick then
                        -- Simula ataque, você pode substituir para chamar função real do jogo
        ...
        if state.AutoStats then
            -- Chamar função para distribuir stats automaticamente
            -- A depender da API do jogo, enviar evento para distribuir
            -- Exemplo (pseudocódigo):
            -- game:GetService("ReplicatedStorage").Remotes.AddStats:FireServer(state.StatType, state.StatAmount)
        end
    end
end)
